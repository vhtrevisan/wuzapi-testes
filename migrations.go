...
var migrations = []Migration{
	...
	{
		ID:    9,
		Name:  "create_chatwoot_config",
		UpSQL: createChatwootConfigSQL,
	},
	{
		ID:    10,
		Name:  "create_chatwoot_conversations",
		UpSQL: createChatwootConversationsSQL,
	},
	{
		ID:    11,
		Name:  "create_chatwoot_messages",
		UpSQL: createChatwootMessagesSQL,
	},
}
...
const createChatwootConversationsSQL = `
-- PostgreSQL version
DO $$
BEGIN
...
const createChatwootMessagesSQL = `
-- PostgreSQL version
DO $$
BEGIN
...
func applyMigration(db *sqlx.DB, migration Migration) error {
	...
	if migration.ID == 1 {
		...
		if db.DriverName() == "sqlite" {
			// Create chatwoot_config table for SQLite
			...
			if err == nil {
				// Create index for faster lookups (partial index for enabled = 1)
				_, err = tx.Exec(`
					CREATE INDEX IF NOT EXISTS idx_chatwoot_config_enabled 
					ON chatwoot_config (enabled) WHERE enabled = 1`)
			}
		} else {
			_, err = tx.Exec(migration.UpSQL)
		}
		...
		if db.DriverName() == "sqlite" {
			// Create chatwoot_conversations table for SQLite
			...
			if err == nil {
				// Create indexes for faster lookups
				_, err = tx.Exec(`
					CREATE INDEX IF NOT EXISTS idx_chatwoot_conversations_user 
					ON chatwoot_conversations (user_id)`)
			}
			if err == nil {
				_, err = tx.Exec(`
					CREATE INDEX IF NOT EXISTS idx_chatwoot_conversations_chat_jid 
					ON chatwoot_conversations (chat_jid)`)
			}
			if err == nil {
				_, err = tx.Exec(`
					CREATE INDEX IF NOT EXISTS idx_chatwoot_conversations_conversation_id 
					ON chatwoot_conversations (chatwoot_conversation_id)`)
			}
		} else {
			_, err = tx.Exec(migration.UpSQL)
		}
		...
		if db.DriverName() == "sqlite" {
			// Create chatwoot_messages table for SQLite
			...
			if err == nil {
				// Create indexes for faster lookups
				_, err = tx.Exec(`
					CREATE INDEX IF NOT EXISTS idx_chatwoot_messages_user 
					ON chatwoot_messages (user_id)`)
			}
			if err == nil {
				_, err = tx.Exec(`
					CREATE INDEX IF NOT EXISTS idx_chatwoot_messages_message_id 
					ON chatwoot_messages (message_id)`)
			}
			if err == nil {
				_, err = tx.Exec(`
					CREATE INDEX IF NOT EXISTS idx_chatwoot_messages_chatwoot_id 
					ON chatwoot_messages (chatwoot_message_id)`)
			}
		} else {
			_, err = tx.Exec(migration.UpSQL)
		}
